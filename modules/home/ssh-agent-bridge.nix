  ...
  home.file.".local/bin/ssh-agent-bridge" = {
    ...
    text = ''
      ...
      agent_is_healthy() {
        ...
        if [ -z "$candidate" ] || [ "$candidate" = "$stable_agent_sock" ] || [ ! -S "$candidate" ]; then
          return 1
        fi
      ...
      current_agent_sock="''${SSH_AUTH_SOCK:-}"
      selected_agent_sock=""

      case "$mode" in
        remote)
          selected_agent_sock="$(first_healthy_agent "$switcher_agent_sock" "$current_agent_sock" "$op_agent_sock" 2>/dev/null || true)"
          ;;
        local)
          selected_agent_sock="$(first_healthy_agent "$op_agent_sock" "$switcher_agent_sock" "$current_agent_sock" 2>/dev/null || true)"
          ;;
        *)
