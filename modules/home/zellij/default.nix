...
in
{
  ...
  # Create borderless-left layout for development with Claude
  xdg.configFile."zellij/layouts/borderless-left.kdl".source = pkgs.writeText "zellij-layout-borderless-left.kdl" ''
    layout {
        default_tab_template {
            pane split_direction="vertical" {
                // Leftmost pane - NO BORDER (neovim)
                pane command="nvim" size="70%" borderless=true

                // Right pane - with border (claude)
                pane command="claude" focus=true size="30%" {
                    args "--append-system-prompt" "\"$(cat ~/.claude/system-prompt.md)\""
                }
            }
            pane size=1 borderless=true {
                plugin location="file://${zjstatus}/zjstatus.wasm" {
